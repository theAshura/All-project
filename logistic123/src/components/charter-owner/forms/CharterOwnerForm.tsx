...
import { clearCharterOwnerErrorsReducer } from '../../../store/charter-owner/charter-owner.action';
import {
  CharterOwner,
...
interface CharterOwnerFormProps {
  isEdit: boolean;
  isCreate?: boolean;
  data: CharterOwnerDetailResponse;
  onSubmit: (CreateCharterOwnerParams) => void;
}
...
const CharterOwnerForm: FC<CharterOwnerFormProps> = ({
  isEdit,
  data,
  onSubmit,
  isCreate,
  ...
  const { errorList, loading } = useSelector((state) => state.charterOwner);
  ...
  useEffect(() => {
    ...
    return () => {
      dispatch(clearCharterOwnerErrorsReducer());
    };
...
export default CharterOwnerForm;
