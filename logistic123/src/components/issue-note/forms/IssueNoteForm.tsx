...
import { clearIssueNoteErrorsReducer } from '../../../store/issue-note/issue-note.action';
import {
  IssueNote,
...
interface IssueNoteFormProps {
  isEdit: boolean;
  isCreate?: boolean;
  data: IssueNoteDetailResponse;
  onSubmit: (CreateIssueNoteParams) => void;
}
...
const IssueNoteForm: FC<IssueNoteFormProps> = ({
  isEdit,
  data,
  onSubmit,
  isCreate,
  ...
  const { errorList, loading } = useSelector((state) => state.issueNote);
  ...
  const handleSubmitAndNew = (data: IssueNote) => {
    const dataNew: IssueNote = { ...data, isNew: true, resetForm };
    onSubmit(dataNew);
  };
  ...
  useEffect(() => {
    ...
    return () => {
      dispatch(clearIssueNoteErrorsReducer());
    };
