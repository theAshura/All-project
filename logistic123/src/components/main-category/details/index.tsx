...
export default function MainCategoryManagementDetailContainer() {
  const { search } = useLocation();
  const dispatch = useDispatch();

  const { loading, mainCategoryDetail } = useSelector(
  ...
  const handleSubmit = useCallback(
    (formData: CreateMainCategoryParams) => {
      dispatch(updateMainCategoryActions.request({ id, data: formData }));
    },
    [dispatch, id],
  );

  useEffect(() => {
    dispatch(getMainCategoryDetailActions.request(id));
    return () => {
      dispatch(
        getMainCategoryDetailActions.success({
          id: '',
          code: '',
          name: '',
          scope: 'internal',
  ...
  return (
    ...
    >
      {({ hasPermission }) =>
        hasPermission ? (
          <div className={styles.getMainCategoryById}>
            <Container>
                      ...
                      {({ hasPermission }) =>
                        hasPermission && (
                          <Button
                            ...
                            onClick={(e) => {
                              history.push(
                                `${AppRouteConst.getMainCategoryById(id)}${
                                  CommonQuery.EDIT
                                }`,
                              );
                            }}
