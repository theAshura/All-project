...
import { clearMainCategoryErrorsReducer } from 'store/main-category/main-category.action';
...
interface MainCategoryFormProps {
  isEdit: boolean;
  isCreate?: boolean;
  data: MainCategoryDetailResponse;
  onSubmit: (CreateMainCategoryParams) => void;
}
...
const MainCategoryForm: FC<MainCategoryFormProps> = ({
  isEdit,
  data,
  onSubmit,
  isCreate,
  ...
  const { errorList, loading } = useSelector((state) => state.mainCategory);
  ...
  useEffect(() => {
    ...
    return () => {
      dispatch(clearMainCategoryErrorsReducer());
    };
