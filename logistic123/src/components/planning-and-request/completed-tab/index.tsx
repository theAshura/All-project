...
import { dateStringComparator, populateStatus } from 'helpers/utils.helper';
...
export default function PlanningAndRequestList(props: IPlanningList) {
  ...
  const { loading, listPlanningCompleted, params, dataFilter } = useSelector(
  ...
  const el = document.getElementById('ag-grid-table-planning-complete');
  const el2 = document.getElementById('ag-grid-table-theme-planning-complete');
        ...
        handleSuccess: () => {
          dispatch(
            getListTemplateActions.request({
              content: MODULE_TEMPLATE.planningCompletedTab,
            }),
          );
        },
      });
  ...
  const dataTable = useMemo(
    () =>
      listPlanningCompleted?.data?.map((item) => ({
        ...
        isLeadAuditor: userInfo?.id === item?.leadAuditorId || DATA_SPACE,
      })) || [],
    [listPlanningCompleted?.data, userInfo],
  );
    ...
    () => [
      {
        ...
        cellRendererFramework: (params) => {
          ...
          const editPermission = allowEditItem(data);
  ...
  const extensionOptions: ExtensionOption[] = [
        ...
        if (listTemplate2?.data?.length > 0) {
          dispatch(
            updateTemplateActions.request({
              ...
              module: MODULE_TEMPLATE.planningCompletedTab,
    ...
    },
    {
      ...
      onClick: () =>
        gridApi?.exportDataAsExcel({ fileName: 'Planning Completed.xlsx' }),
    },
    {
      label: TEXT_ACTION_TEMPLATE.CSV,
      icon: images.icons.agGrid.icAGCsv,
      onClick: () =>
        gridApi?.exportDataAsCsv({ fileName: 'Planning Completed.csv' }),
    },
    !isFullScreen && {
    ...
    <div className={styles.container}>
      ...
      <div className={styles.wrapTable}>
        <AGGridTable
          ...
          totalItem={listPlanningCompleted?.totalItem}
          ...
          renderModalInside={
            <>
              ...
              <ModalList
                ...
                isOpen={modalOpen}
                toggle={setModalOpen}
                templateModule={MODULE_TEMPLATE.planningCompletedTab}
              />
            </>
          }
        />
