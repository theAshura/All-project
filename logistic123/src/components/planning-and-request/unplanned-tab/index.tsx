...
export default function PlanningAndRequestList(props: IPlanningList) {
  ...
  const { loading, listPlanningUnplanned, params, dataFilter } = useSelector(
  ...
  const { listTemplate3, templateDetail3 } = useSelector(
  ...
  const el = document.getElementById('ag-grid-table-planning-unplanned');
  const el2 = document.getElementById('ag-grid-table-theme-planning-unplanned');
  ...
  const handleSaveFilter = (data: CommonApiParam) => {
  ...
  const handleDeletePlanningAndRequest = (id: string) => {
    dispatch(
  ...
  useEffect(() => {
    if (listTemplate3?.data?.length === 0) {
  ...
  useEffect(() => {
    if (templateDetail3 && listTemplate3?.data?.length > 0) {
      setTemplateID(templateDetail3.id);
      if (!dataFilter || !isFirstSetDataFilter) {
        setPage(0);
        setPageSize(20);
        const template = JSON.parse(templateDetail3.template);
        // eslint-disable-next-line no-unused-expressions
        ...
        handleSuccess: () => {
          dispatch(
            getListTemplateActions.request({
              content: MODULE_TEMPLATE.planningUnplannedTab,
            }),
          );
        },
      });
  ...
  const dataTable = useMemo(
    () =>
      listPlanningUnplanned?.data?.map((item) => ({
    ...
    [listPlanningUnplanned?.data, userInfo],
  );
  ...
  const extensionOptions: ExtensionOption[] = [
        ...
        if (listTemplate3?.data?.length > 0) {
          dispatch(
            updateTemplateActions.request({
              id: templateDetail3.id,
              module: MODULE_TEMPLATE.planningUnplannedTab,
              type: templateDetail3.type,
              name: templateDetail3.name,
    ...
    },
    {
      ...
      onClick: () =>
        gridApi?.exportDataAsExcel({ fileName: 'Planning Unplanned.xlsx' }),
    },
    {
      label: TEXT_ACTION_TEMPLATE.CSV,
      icon: images.icons.agGrid.icAGCsv,
      onClick: () =>
        gridApi?.exportDataAsCsv({ fileName: 'Planning Unplanned.csv' }),
    },
    !isFullScreen && {
  ...
  const getTypeTemplate = (type: string, templateDetail3Type: string) => {
    ...
    return templateDetail3Type || TYPE_TEMPLATE.template;
  };
  ...
  const handleSaveTemplate = (templateName: string) => {
    dispatch(
      createTemplateActions.request({
        module: MODULE_TEMPLATE.planningUnplannedTab,
        type: getTypeTemplate(titleModal, templateDetail3?.type),
        name: templateName,
        defaultTemplate: JSON.stringify({
          columns: gridColumnApi?.getColumnState(),
          filterModel: currentFilterModel,
    ...
    <div className={styles.container}>
      ...
      <div className={styles.wrapTable}>
        <AGGridTable
          ...
          totalItem={listPlanningUnplanned?.totalItem}
          ...
          renderModalInside={
            <>
              ...
              />
              <ModalList
                data={listTemplate3?.data || []}
                isOpen={modalOpen}
                toggle={setModalOpen}
                templateModule={MODULE_TEMPLATE.planningUnplannedTab}
              />
            </>
          }
        />
