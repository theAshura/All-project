...
export default function PscActionManagementDetailContainer() {
  const { search } = useLocation();
  const dispatch = useDispatch();

  const { loading, pscActionDetail } = useSelector((state) => state.pscAction);
  ...
  const handleSubmit = useCallback(
    (formData: CreatePscActionParams) => {
      dispatch(updatePscActions.request({ id, data: formData }));
    },
    [dispatch, id],
  );

  useEffect(() => {
    dispatch(getPscActionDetailActions.request(id));
    return () => {
      dispatch(
        getPscActionDetailActions.success({
          id: '',
          code: '',
          name: '',
          scope: 'internal',
  ...
  return (
    ...
    >
      {({ hasPermission }) =>
        hasPermission ? (
          <div className={styles.getPscActionById}>
            <Container>
                      ...
                      {({ hasPermission }) =>
                        hasPermission && (
                          <Button
                            ...
                            onClick={(e) => {
                              history.push(
                                `${AppRouteConst.getPscActionById(id)}${
                                  CommonQuery.EDIT
                                }`,
                              );
                            }}
