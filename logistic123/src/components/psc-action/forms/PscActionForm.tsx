...
import { clearPscActionErrorsReducer } from '../../../store/psc-action/psc-action.action';
import {
  PscAction,
...
interface PscActionFormProps {
  isEdit: boolean;
  isCreate?: boolean;
  data: PscActionDetailResponse;
  onSubmit: (CreatePscActionParams) => void;
}
...
const PscActionForm: FC<PscActionFormProps> = ({
  isEdit,
  data,
  onSubmit,
  isCreate,
  ...
  const { errorList, loading } = useSelector((state) => state.pscAction);
  ...
  const handleSubmitAndNew = (data: PscAction) => {
    const dataNew: PscAction = { ...data, isNew: true, resetForm };
    onSubmit(dataNew);
  };
  ...
  useEffect(() => {
    ...
    return () => {
      dispatch(clearPscActionErrorsReducer());
    };
