...
interface ModalPscDeficiencyProps {
  isOpen?: boolean;
  isCreate?: boolean;
  title?: string;
...
const ModalPscDeficiency: FC<ModalPscDeficiencyProps> = (props) => {
  const {
    loading,
    isCreate,
    toggle,
  ...
  const { errorList } = useSelector((state) => state.pscDeficiency);
            ...
            if (res.data.isExist) {
              switch (field) {
                case 'code':
                  if (value !== data?.code) {
                    setError(field, {
                      message: t('pscDeficiencyCodeIsExisted'),
                    });
                  }
                  break;
                case 'name':
                  if (value !== data?.name) {
                    setError(field, {
                      message: t('pscDeficiencyNameIsExisted'),
                    });
                  }
                  break;
                default:
        ...
        <div>
          <Row className="pt-2 mx-0 pb-3">
            <Col className="ps-0 d-flex align-items-center" md={4} xs={4}>
              <LabelUI label={t('txPSCCode')} isRequired />
            </Col>
          ...
          <Row className="pt-2 mx-0 pb-3">
            <Col className="ps-0  d-flex align-items-center" md={4} xs={4}>
              <LabelUI label={t('txPSCName')} isRequired />
            </Col>
            <Col className="px-0" md={8} xs={8}>
              <Input
                {...register('name')}
                ...
                placeholder={t('placeHolder.txPSCName')}
...
export default ModalPscDeficiency;
