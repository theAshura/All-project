...
export default function RiskFactorManagementDetailContainer() {
  const { search } = useLocation();
  const dispatch = useDispatch();

  const { loading, riskFactorDetail } = useSelector(
  ...
  const handleSubmit = useCallback(
    (formData: CreateRiskFactorParams) => {
      dispatch(updateRiskFactorActions.request({ id, data: formData }));
    },
    [dispatch, id],
  );

  useEffect(() => {
    dispatch(getRiskFactorDetailActions.request(id));
    return () => {
      dispatch(
        getRiskFactorDetailActions.success({
          id: '',
          code: '',
          name: '',
          scope: 'internal',
  ...
  return (
    ...
    >
      {({ hasPermission }) =>
        hasPermission ? (
          <div className={styles.getRiskFactorById}>
            <Container>
                      ...
                      {({ hasPermission }) =>
                        hasPermission && (
                          <Button
                            ...
                            onClick={(e) => {
                              history.push(
                                `${AppRouteConst.getRiskFactorById(id)}${
                                  CommonQuery.EDIT
                                }`,
                              );
                            }}
