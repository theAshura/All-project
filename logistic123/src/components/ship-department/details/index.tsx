...
import ShipDepartmentForm from '../forms/ShipDepartmentForm';

export default function ShipDepartmentManagementDetailContainer() {
  const { search } = useLocation();
  const dispatch = useDispatch();

  const { loading, shipDepartmentDetail } = useSelector(
  ...
  const handleSubmit = useCallback(
    (formData: CreateShipDepartmentParams) => {
      dispatch(updateShipDepartmentActions.request({ id, data: formData }));
    },
    [dispatch, id],
  );

  useEffect(() => {
    dispatch(getShipDepartmentDetailActions.request(id));
    return () => {
      dispatch(
  ...
  return (
    ...
    >
      {({ hasPermission }) =>
        hasPermission ? (
          <div className={styles.getShipDepartmentById}>
            <Container>
                      ...
                      {({ hasPermission }) =>
                        hasPermission && (
                          <Button
                            ...
                            onClick={(e) => {
                              history.push(
                                `${AppRouteConst.getShipDepartmentById(id)}${
                                  CommonQuery.EDIT
                                }`,
                              );
                            }}
