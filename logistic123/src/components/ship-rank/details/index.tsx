...
export default function ShipRankManagementDetailContainer() {
  const { search } = useLocation();
  const dispatch = useDispatch();

  const { loading, ShipRankDetail } = useSelector((state) => state.shipRank);
  ...
  const handleSubmit = useCallback(
    (formData: CreateShipRankParams) => {
      dispatch(updateShipRankActions.request({ id, data: formData }));
    },
    [dispatch, id],
  );

  useEffect(() => {
    dispatch(getShipRankDetailActions.request(id));
    return () => {
      dispatch(
        getShipRankDetailActions.success({
          id: '',
          createdAt: undefined,
          updatedAt: undefined,
          code: '',
  ...
  return (
    ...
    >
      {({ hasPermission }) =>
        hasPermission ? (
          <div className={styles.shipRankDetail}>
            <Container>
                      ...
                      {({ hasPermission }) =>
                        hasPermission && (
                          <Button
                            ...
                            onClick={(e) => {
                              history.push(
                                `${AppRouteConst.getShipRankById(id)}${
                                  CommonQuery.EDIT
                                }`,
                              );
                            }}
