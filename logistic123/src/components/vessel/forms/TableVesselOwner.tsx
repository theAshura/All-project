...
import { ModalVesselOwner } from './ModalVesselOwner';
import {
  checkTimeLine,
...
export const TableVesselOwner: FC<Props> = ({
  loading,
  value,
  onchange,
  disable,
  ...
  const [vesselOwners, setVesselOwners] = useState(value || []);
  ...
  const onSubmitForm = useCallback(
    (data) => {
      const newVesselOwner = handleDataCompanyUpdate(
        vesselOwners,
        data,
        index,
        !initialData?.length,
      );
      onchange(newVesselOwner);
      setModal(false);
    },
    [vesselOwners, index, initialData?.length, onchange],
  );

  const onDeleteVesselOwner = useCallback(
    (index) => {
      const newVesselOwner = vesselOwners ? [...vesselOwners] : [];
      newVesselOwner.splice(index, 1);
      onchange(newVesselOwner);
    },
    [vesselOwners, onchange],
  );
  ...
  const handleDelete = useCallback(
    (index) => {
      showConfirmBase({
        ...
        onPressButtonRight: () => onDeleteVesselOwner(index),
      });
    },
    [dynamicLabels, onDeleteVesselOwner],
  );
  ...
  const renderRow = useCallback(
    (isScrollable?: boolean) => {
      if (!loading && vesselOwners?.length > 0) {
        return (
          <tbody>
            {vesselOwners?.map((item, index) => {
  ...
  useEffect(() => {
    setVesselOwners(handleSortByDate(value));
  }, [value]);
      ...
      <div className={cx(styles.containerForm)}>
        <div className="d-flex justify-content-between align-items-center">
          <div className={cx(styles.titleContainer)}>
            {renderDynamicLabel(
              dynamicLabels,
              VESSEL_DETAIL_DYNAMIC_FIELDS['Vessel owner'],
            )}
          </div>
          {!disable && (
            <Button
